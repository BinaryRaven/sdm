...
void draw_bigclock(struct term_buf *buf) {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    alpha_blit(tb_cell_buffer(), xo + i * (CLOCK_W + 1), yo, CLOCK_W, CLOCK_H,
               clockcell);
    free(clockcell);
  }
...
void draw_labels(struct term_buf *buf) // throws
{
  ...
  if (dgn_catch()) {
    ...
    tb_blit(buf->box_x + config.margin_box_h,
            buf->box_y + config.margin_box_v + 4, strlen(lang.login), 1, login);
    free(login);
  }
...
void draw_desktop(struct desktop *target) {
  ...
  tb_change_cell(target->x + target->visible_len - 1, target->y, '>', config.fg,
                 config.bg);

  for (uint16_t i = 0; i < len; ++i) {
    tb_change_cell(target->x + i + 2, target->y, target->list[target->cur][i],
                   config.fg, config.bg);
  }
}
